...
1. Random Split or Time Split: Divides the drug-target interactions into an 80/20 training and test set or
use the cutoff year which is 2018.
2. Chemical Similarity (Drugs): Uses ChemBERTa embeddings to calculate max Cosine similarity.
3. Sequence Similarity (Proteins): Uses ESM2 embeddings to calculate max Cosine similarity.
4. Aggressive Filtering: Iterates through various similarity thresholds.
5. Retraining & Evaluation: Retrains the Random Forest on these progressively isolated sets.
6. Pareto Visualization: Generates a trade-off plot comparing AUC loss against data reduction.
"""
...
# OUT_DIR = PROJECT_ROOT / "artifacts" / "rf_similarity_random2"
OUT_DIR = PROJECT_ROOT / "artifacts" / "rf_similarity_time2"
...
def load_data():
    ...
    train_int = interactions[interactions["year"] <= CUTOFF].copy().reset_index(drop=True)
    test_int = interactions[interactions["year"] > CUTOFF].copy().reset_index(drop=True)
...
def run_experiment(
    ...
    _, X_train, y_train, _ = prepare_feature_matrix(filtered_train, drug_emb, prot_emb)
    _, X_test,  y_test,  _ = prepare_feature_matrix(test_int, drug_emb, prot_emb)
...
def plot_pareto(results_df, baseline_auc):
    ...
    fig.suptitle("Similarity-Based Partitioning: Pareto Analysis", fontsize=13, fontweight="bold")
    ...
    scatter = ax1.scatter(
        df["delta_auc"] * 100, df["delta_pairs"],
        c=df["test_roc_auc"], cmap="RdYlBu",
        vmin=0.5, vmax=baseline_auc, s=80, alpha=0.8, zorder=3,
    )
    ...
    ax1.scatter([0], [0], color="red", s=150, zorder=5, label=f"No filtering (AUC={baseline_auc:.3f})")
    ax1.set_xlabel("Delta AUC ROC (% drop from baseline)", fontsize=11)
    ax1.set_ylabel("Delta Known Pairs (% reduction in training)", fontsize=11)
    ...
    for d_thresh in sorted(df["drug_threshold"].unique()):
        ...
        if len(subset) > 1:
            ax2.plot(
                subset["delta_pairs"], subset["test_roc_auc"],
                marker="o", markersize=5, label=f"Drug sim < {d_thresh}",
                linewidth=1.5, alpha=0.8,
            )

    ax2.axhline(y=baseline_auc, color="red", linewidth=2, linestyle="--", label=f"No filtering ({baseline_auc:.3f})")
    ax2.axhline(y=0.5, color="gray", linewidth=1, linestyle=":", label="Random (0.5)", alpha=0.6)
    ...
    plt.tight_layout()
    out = FIG_DIR / "fig_similarity_partitioning_random2.png"
...
def main():
    ...
    _, X_train_full, y_train_full, _ = prepare_feature_matrix(train_int, drug_emb, prot_emb)
    _, X_test_full,  y_test_full,  _ = prepare_feature_matrix(test_int, drug_emb, prot_emb)
    ...
    for d_thresh in drug_thresholds:
        for p_thresh in prot_thresholds:
            ...
            result = run_experiment(
                train_int, test_int, drug_emb, prot_emb,
                drug_max_sim, prot_max_sim, d_thresh, p_thresh,
            )
            if result:
