...
1. Data Loading: Imports the generated ESM2  and DRKG target embeddings.
2. Alignment: Intersects the targets to find common IDs and sorts both matrices so row 'i'
   corresponds to the exact same ChEMBL target in both datasets.
3. Similarity Calculation: L2-normalizes the matrices and computes the dot product to yield
   n x n pairwise cosine similarity matrices.
4. Hierarchical Clustering: Computes the Euclidean distance and average linkage on the ESM2
   matrix to cluster similar proteins together. It applies this exact same structural ordering
   to the DRKG matrix to ensure a 1:1 visual comparison.
5. Visualization: Plots the two sorted matrices side-by-side as heatmaps, calculates the
   Pearson correlation coefficient between their upper triangles, and saves the image.
"""
...
def main():
    esm2_ids, esm2_emb = load_esm2(PROJECT_ROOT / "data/processed/protein_embeddings.csv")
    drkg_ids, drkg_emb = load_drkg(PROJECT_ROOT / "data/processed/drkg_target_embeddings.csv")
